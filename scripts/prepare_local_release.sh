...
_progress_line() {
    ...
    printf "  %b %3d%%  ${DIM}[%d/%d]  %s${RST}\n" "$bar" "$pct" "$CURRENT_STEP" "$TOTAL_STEPS" "$total_el"
}
...
_spinner() {
    ...
    while kill -0 "$pid" 2>/dev/null; do
        ...
        printf "\r  ${CYN}${frames[$i]}${RST} ${DIM}%-40s %s${RST}" "$label" "$elapsed"
        i=$(( (i + 1) % ${#frames[@]} ))
        sleep 0.1
    done
    printf "\r\033[2K"
...
echo ""
echo -e "  ${BLU}${BOLD}Dicton${RST} — Local Release Builder"
echo -e "  ─────────────────────────────────"
...
echo "$deb_contents" | grep -q "opt/dicton/dicton" \
    || { echo -e "  ${RED}✘${RST} .deb missing /opt/dicton/dicton"; exit 1; }
echo "$deb_contents" | grep -q "usr/bin/dicton" \
    || { echo -e "  ${RED}✘${RST} .deb missing /usr/bin/dicton"; exit 1; }
